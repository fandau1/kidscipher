...
describe("MorseCodeCipher", () => {
  ...
  test("encodes HELLO WORLD with custom dot dash symbol", () => {
    const result = morse.encode("HELLO WORLD", {dotDashMapping: { dot: "*", dash: "_" }, input: { caseSensitive: false, letterSeparator: "", wordSeparator: " " }, output: { casing: "original", letterSeparator: "/", wordSeparator: "/" } });
    expect(result).toBe("****/*/*_**/*_**/___///*__/___/*_*/*_**/_**");
  });

  test("decodes Morse back to text with default options", () => {
    const result = morse.decode("****/*/*_**/*_**/___///*__/___/*_*/*_**/_**", {dotDashMapping: { dot: "*", dash: "_" }, input: { caseSensitive: false, letterSeparator: "/", wordSeparator: "///" }, output: { casing: "lower", letterSeparator: "", wordSeparator: " " } });
    expect(result).toBe("hello world");
  });
});
